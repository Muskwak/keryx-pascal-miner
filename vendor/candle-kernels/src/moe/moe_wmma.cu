...
// bf16 WMMA fragments require sm_80+. On older archs (Turing sm_75, Pascal) the
// kernel body is compiled out to a no-op so candle-kernels still builds and links;
// the MoE (Qwen3-235B) is only ever invoked on sm_80+ rigs (VRAM gate + sm_80 build).
#if __CUDA_ARCH__ >= 800
