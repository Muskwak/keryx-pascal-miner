...
// Per-arch PTX selector generated by build.rs: get_pom_ptx((cc_major, cc_minor)) -> &'static str.
// Every arch's PTX is compiled from the SAME tuned cuda/pom_mine.cu, so sm_61 (P40/1070) keeps
// its full tuning while newer GPUs get native PTX; unknown arches fall back to sm_61 (JITs up).
include!(concat!(env!("OUT_DIR"), "/pom_ptx.rs"));

/// Select the embedded PTX matching a CUDA device ordinal's compute capability. Falls back to
/// SM61 (Pascal) if the query fails — PTX is forward-compatible, so the driver JITs it up.
...
impl PomGpuMiner {
    ...
    pub fn load(gguf_path: &str, device_id: usize) -> candle_core::Result<Self> {
        ...
        // Warm the module cache so mine() never compiles on the hot path. Pick the PTX matching
        // this device's arch (P40/1070 → sm_61 tuned kernel; newer GPUs → their native PTX).
    ...
    pub fn load_shared(
        ...
        let bases_dev = stream.clone_htod(&bases).map_err(candle_core::Error::wrap)?;
        let prefix_dev = stream.clone_htod(&prefix).map_err(candle_core::Error::wrap)?;
        // Zero-dup path is the inference GPU (Pascal-safe sm_61 PTX; the driver JITs it up on
        // newer inference GPUs). On the P40 sm_61 is native, so it keeps the tuned kernel.
    ...
    pub fn mine(&self, pre_pow_hash: &[u8; 32], timestamp: u64, target_le: &[u8; 32], start: u64, batch: u64) -> candle_core::Result<Option<u64>> {
        ...
        // Module already loaded per-device in the constructor; cached by name so the PTX arg here
        // is only a fallback. get_pom_ptx((6,1)) is a valid compatible module for any GPU.
        let func = self.cuda.get_or_load_custom_func("pom_mine", "pom_mine_mod", get_pom_ptx((6, 1)))?;
